function  [a b r] = ellipse_regression(iqData, iterations)
% **********************************************************************
% function ellipse_regression(iqData, iterations)
%
%
% Description:
% -----------
...
%  Loop over the number of iterations to refine the new circle
  for j=1:iterations
    F                   = generate_Fsq(x_points, y_points, circle_average);
    J                   = generate_JacobianSq(x_points, y_points, circle_average);
...
a = fitellip(real(iqData)', imag(iqData)');
drawellip(a, real(iqData)', imag(iqData)');
v   = solveellipse(a);
iq_offset           = 100*sqrt(v(3)^2 + v(4)^2)/v(2)
